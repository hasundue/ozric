...
fn runExactSolutionsExample(allocator: std.mem.Allocator, output_dir: []const u8) !void {
    ...
    // Hard sphere exact solution
    ozric.exact.ExactSolutions.hardSphere(&g_r_exact, &h_r_exact, &c_r_exact, density, sigma);

    std.debug.print("📊 Hard Sphere Exact Solution (Percus-Yevick Analytical):\n", .{});
    ...
    // Test Lennard-Jones perturbation solution
    std.debug.print("📊 Lennard-Jones Perturbation Solution (ε=1.0, σ={d:.1}):\n", .{sigma});
